      ...
      <div class="register1 d-f f-d-c a-i-c">
        ...
        <Form ref="formCustom" :model="formCustom" :rules="ruleCustom" :label-width="80">
          <FormItem label="用户名" prop="username">
            <Input v-model="usernames" placeholder="请输入用户名" class="input" />
          </FormItem>
        ...
        <div class="d-f m-t-10">
          <Checkbox v-model="single">记住密码</Checkbox>
           <span class=" m-l-40">
            ...
            <span class="items " @click="error(false)">忘记密码</span>
            </span>
        </div>
        <Button type="success" class="item1" @click="login">登录</Button>
        <Button type="error" class="item1" @click="logins">返回</Button>
        <div class="item2"></div>
        <div class="d-f m-t-20"  @click="info(false)">
          其它账号登录:
        ...
        this.$api
          ...
          .then(res => {
             if (res.code===200) {
              ...
              localStorage.setItem("username", this.usernames);
            } 
            console.log(res);
          })
          .catch(err => {
