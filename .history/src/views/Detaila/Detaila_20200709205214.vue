<template>
  <div>
    <div class="details d-f">
      <div class="d-f f-d-c detailsa">
        <div v-for="(item,index) in obj.productImageSmall" :key="index" @click="clickImg(item)">
          <div class="details1 d-f j-c-c a-i-c m-t-10 c-p">
            <img :src="item" alt width="54" height="54" />
          </div>
        </div>
      ...
      <div class="m-l-40 detailsa">
        ...
        <div class="d-f m-t-30">
          ...
          <el-input-number size="mini" v-model="num4" :min="1" :max="10" class="num" @click="input(item)"></el-input-number>
        </div>
        <div class="m-t-30 sale"></div>
        <div class="d-f m-t-30">
          <Button size="large" type="primary" class="large1" @click="shopping">加入购物车</Button>
          <Button size="large" type="default" class="large" @click="toshopping">立即购买</Button>
        </div>
      </div>
    </div>
    <div class="products">
...
export default {
  ...
  methods: {
    ...
    shopping(){
      this.$api.addCart(this.obj.productId).then(res=>{
        console.log(res);
      }).catch(err=>{
        console.log(err);
      })
