    ...
    <div class="shopping3">
        ...
        <div class="d-f shopping5 m-l-20">
          ...
          <div class="d-f shopping6">
            ...
            <el-input-number
              ...
              @change="handleChange"
            ></el-input-number>
            <div class="shopping1">{{item.count*item.salePrice | fixed}}</div>
            <Icon type="md-close-circle" class="circle" @click="circle(item._id)" />
          </div>
        </div>
      </div>
      ...
      <div class="d-f j-c-s-b font2 a-i-c">
        <div class="d-f m-t-20 m-l-20">
          ...
          <div class="f-c-hu f-s-12" @click="clickItem">删除选中的商品</div>
        </div>
        <div class="d-f shopping9">
          <div>
            <div class="f-s-16">
              已选中
              <span class="f-c-ho m-l-10 f-s-16">{{totalCounts}}</span>
              <span class="m-l-10 f-s-16">件商品</span>
            </div>
            <div class="f-s-16">
              共计
              <span class="font f-s-16">{{totalCount}}</span>
          ...
          <Button type="primary" class="buttom" @click="primary">现在结算</Button>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    // 删除全部
    clickItem(){
         this.asd = this.arr.filter(item => {
        return item.check === true;
      })
      this.asd.map(item1=>{
         this.$api.editCart(item1._id).then(res=>{
           console.log(res);
         }).catch(err=>{
           console.log(err);
         })
    ...
    primary(){
      ...
      this.$router.push({name:'Settlement',query:{msg:this.msg,}});
      
      }else{
        this.$Message.error('您还没有勾选商品，请添加');
      }
