    ...
    <div class="register d-f j-c-c a-i-c">
      <div class="register1 d-f f-d-c a-i-c">
        ...
        <Form ref="formCustom" :model="formCustom" :rules="ruleCustom" :label-width="80" class="m-t-10">
          ...
          <FormItem label="再次输入" prop="rpassword">
            <Input type="password" v-model="formCustom.rpassword" placeholder="请输入密码" class="input"/>
          </FormItem>
        </Form>
            <div class="vpat">
        ...
        <Button type="success" class="item1" @click="clickz">注册</Button>
        <div class="item2"></div>
...
export default {
  ...
  methods: {
    ...
    clickz(){
       this.$api.register({ username:this.formCustom.username, password: this.formCustom.password, })
       .then(res => {
        if (this.password !== this.rpassword) {
            ...
            this.$Message.error(res.msg);
          } else if(this.singles===false){
              this.$Message.success(必须点击);
          } else {
